...
class _BuyerProductCardState extends State<BuyerProductCard> {                                            
  ...
  void _addToCart() async {                              
    ...
    try {                                                  
      ...
      // 🟢 [التصحيح الرئيسي]: استخدام الوسائط المسماة (Named Arguments) بدلاً من الموضعية (Positional)
      await cartProvider.addItemToCart(
        productId: widget.productId,                                  // 1. معرف المنتج (ProductId)
        name: widget.productData['name'] ?? 'منتج غير معروف',         // 2. اسم المنتج (Name)
        offerId: selectedOffer.offerId!,                              // 3. معرف العرض (OfferId)
        sellerId: selectedOffer.sellerId!,                            // 4. معرف البائع (SellerId)
        sellerName: selectedOffer.sellerName!,                        // 5. اسم البائع (SellerName)
        price: selectedOffer.price,                                  // 6. السعر (Price)
        unit: selectedOffer.unitName,                                 // 7. اسم الوحدة (Unit)
        unitIndex: selectedOffer.unitIndex ?? 0,                      // 8. مؤشر الوحدة (UnitIndex)
        quantityToAdd: currentQuantity,                               // 9. الكمية المراد إضافتها (QuantityToAdd)
        imageUrl: imageUrl,                                           // 10. رابط الصورة (ImageUrl)
      );
